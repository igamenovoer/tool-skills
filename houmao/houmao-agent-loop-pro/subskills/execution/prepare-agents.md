...
5. Treat `houmao-mgr project ...` as the underlying surface owned by `houmao-agent-definition`, not as loop-local preparation logic.
...
- Do not reimplement specialist creation, project-profile mutation, launch-dossier mutation, credential defaulting, or launcher selection; route to `houmao-agent-definition`.
